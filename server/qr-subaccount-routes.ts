...
async function handleDispatchSubmit(
  ...
  let txCode = order.transactionCode;
  if (!txCode) {
    txCode = await generateTransactionCode(req.db, businessDate);
  }

  await req.db.update(orders)
    .set({ transactionCode: txCode, orderMode: "DISPATCH", dispatchStatus: "PENDING_PAYMENT" })
  ...
  if (subaccountId) {
    const [sa] = await req.db.select().from(orderSubaccounts).where(eq(orderSubaccounts.id, subaccountId));
    subaccount = sa || null;
  }
  ...
  broadcastFn("order_updated", { tableId: table.id, orderId: order.id });
  broadcastFn("table_status_changed", { tableId: table.id });
