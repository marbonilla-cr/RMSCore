...
export async function onOrderItemsConfirmedSent(
  ...
  try {
    ...
    for (const orderItemId of orderItemIds) {
      await processOneDeduction(client, orderId, orderItemId, userId);
    }
...
async function processOneDeduction(
  ...
  if (existingRows.length > 0) {
    ...
    if (existing.status === "REVERSED") return;
    if (existing.status === "CONSUMED" && existing.basic_deducted_at !== null) return;
...
export async function onOrderItemsVoided(
  ...
  try {
    await client.query("BEGIN");
    for (const orderItemId of orderItemIds) {
      await processOneReversal(client, orderItemId, userId);
    }
    await client.query("COMMIT");
  } catch (err) {
    try { await client.query("ROLLBACK"); } catch (_) {}
...
async function processOneReversal(
  ...
  if (dedRows.length === 0 || dedRows[0].status === "REVERSED") return;
