...
-   **Table Soft-Delete:** Tables can be archived (soft-deleted) via PATCH `/api/admin/tables/:id/archive`. Sets `deletedAt` timestamp, appends `[DEL-{base36timestamp}]` suffix to name/code to free originals for reuse, and marks inactive. `getAllTables()` and `getTableByCode()` filter deleted tables by default. Admin UI shows red trash icon with confirmation dialog.
