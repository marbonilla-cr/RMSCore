...
- **QR Client Page**: Dual-mode interface. Easy Mode: drill-down category navigation with 2-column grid layout (6 items/page), large touch targets (min-h-[80px] buttons) for elderly users. Standard Mode: accordion category layout matching waiter interface (expand/collapse categories, product rows with name/price/qty). Both support subaccount selection, customer name input ("Como te llamas?"), modifier selection, and quantity adjustment. Spanish microcopy: "Tranqui: un salonero confirma tu pedido antes de mandarlo a cocina"
- **Easy Mode Filtering**: `easyMode` boolean on `categories` and `products` tables. Admin pages have "Modo Fácil" toggle switches. QR menu endpoint filters by `?mode=easy` to show only easyMode=true products in easyMode=true categories. Fewer items = simpler navigation for elderly users.
- **Subaccount Grid**: Numbered slot buttons (1 to maxSubaccounts) in 2-column grid. Clicking creates or selects the specific slot. Backend validates slot bounds. `maxSubaccounts` served from `/api/qr/:tableCode/info`.
