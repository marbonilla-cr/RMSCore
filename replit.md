...
-   **Multi-Tenant Migration System:** Versioned SQL migrations in `migrations/` directory. `schema_migrations` table tracks applied migrations per schema. `propagateMigrations()` runs on startup, auto-applies pending migrations to all active tenant schemas. `getMigrationStatus()` provides per-schema dashboard. Files: `server/provision/migrate-tenants.ts`, `server/provision/run-migrate.ts`. NEVER use `drizzle-kit push` — it will drop multi-tenant tables. Instead: add SQL to `migrations/` and let auto-propagation handle it.
