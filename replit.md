...
- **QR Client Page**: Dual-mode interface. Easy Mode: drill-down category navigation with 2-column grid layout (6 items/page), large touch targets (min-h-[80px] buttons) for elderly users. Standard Mode: card grid per diner with food/drink separation. Both support subaccount selection, customer name input ("Como te llamas?"), modifier selection, and quantity adjustment. Spanish microcopy: "Tranqui: un salonero confirma tu pedido antes de mandarlo a cocina"
